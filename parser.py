...
def main(category_href): # Parse chosen category
	...
	global sql_global
	sql_global = codecs.open("sql_global.sql", "w+", "utf-8")
...
def parse_inside_page(html_doc_inside_href, img_global_counter): # Parse particular page
	...
	excerpt = clear_white_space(try_except('.objectosSubBlock', soup_inside), "\s", 3)
	main_text = clear_white_space(try_except('.text_body', soup_inside), "\s", 3)

	print(html_doc_inside_href) # Catching possible errors

	main_img_node = content_div.select_one('img')

	try: # trying to parse main image's srcset || src
		href_array.append((str(main_img_node.get('srcset')).split()[-2]))
	except:
		href_array.append(main_img_node.get('src'))

	href_array.extend(list(map(lambda x: x.get('href'), content_div.select('.text_body a[href^="https://nn-vent.ru/wp-content/uploads/"]'))))
	...
	sql_global.write(sql_template(excerpt, main_text, replace_single_quote(clear_white_space(title, '\n', 0)), title_translaterate, post_global_counter, img_array))
...
def clear_white_space(text, method="\s", num_of_space=0): # Clear white space 
...
def save_in_folder(post_global_counter, img_global_counter, file_link, folder_name, file_name, img_counter): # Save img in page related folder
	...
	save_in_global_folder(img_global_counter, file_link, file_name, img_counter)


def save_in_global_folder(img_global_counter, file_link, file_name, img_counter): # Save img in stock folder
...
def sql_template(excerpt, main_text, post_title, post_name, post_global_counter, img_array): # building SQL-request for new page
	
	# Create content with right amount if images
	content_inner = """[vc_row][vc_column][vc_row_inner][vc_column_inner width="1/2"][vc_single_image image="{}" img_size="full" alignment="center" onclick="link_image"][/vc_column_inner][vc_column_inner width="1/2"][vc_column_text]{}[/vc_column_text][/vc_column_inner][/vc_row_inner][vc_empty_space height="40px"][vc_column_text]{}[/vc_column_text][vc_empty_space height="40px"][vc_custom_heading text="ВЫПОЛНЕННЫЕ РАБОТЫ" font_container="tag:h2|text_align:left|color:%230079ce" use_theme_fonts="yes" css=".vc_custom_1538483422586{{border-left-width: 3px !important;padding-left: 15px !important;border-left-color: #0079ce !important;border-left-style: solid !important;}}"][vc_empty_space height="35px"][ultimate_carousel slides_on_desk="2" slides_on_tabs="2" slides_on_mob="1" autoplay="off" next_icon="ultsl-arrow-right6" prev_icon="ultsl-arrow-left6" dots_icon="ultsl-radio-unchecked"]""".format(img_array[0], excerpt, main_text)

	for img_id in img_array[1:]:
		content_inner += """[vc_single_image image="{}" img_size="full" alignment="center" onclick="link_image"]""".format(img_id)

	content_inner += """[/ultimate_carousel][vc_empty_space height="15px"][/vc_column][/vc_row]"""
	...
	""".format(post_title, post_name, excerpt[:200]+"...", content_inner)

	return sql_request


if __name__ == '__main__':
	main(str(input("Type link: "))) # link of sites categoty page (https://nn-vent.ru/objects/nashi-klientyi/)